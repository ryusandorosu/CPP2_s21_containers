#ifndef SRC_S21_CONTAINERS_TREE_H_
#define SRC_S21_CONTAINERS_TREE_H_
...
class AvlTree {
 public:
  ...
  std::pair<iterator, bool> InsertOrAssign(const value_type &obj) {
    iterator node = Find(obj.first);
    if (node != end()) {
      node.getCurrent()->data.second = obj.second;
      return {node, false};
    } else {
      Insert(obj);
      return {Find(obj.first), true};
    }
  }
  ...
  iterator FindNum(const Key &key) {
    Node *node = root;
    while (node != nullptr) {
      if (key < node->data) {
        node = node->left;
...
#endif  // SRC_S21_CONTAINERS_TREE_H_